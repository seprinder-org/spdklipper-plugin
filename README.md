...
- **Hiển thị trạng thái SPD trên Fluidd/Mainsail**: Hiển thị `machine_id` và trạng thái kết nối (`ACTIVE`/`INACTIVE`) qua M117 trên thanh tiêu đề giao diện.
...
- Cài đặt Moonraker custom component (`spd_status.py`) để hiển thị trạng thái kết nối SPD qua M117
...
### Hiển thị trạng thái SPD trên Fluidd/Mainsail

Plugin tự động hiển thị trạng thái kết nối SPD trên thanh tiêu đề của Fluidd/Mainsail thông qua lệnh `M117` (display message).

**Cách hoạt động:**

1. Plugin ghi file trạng thái `~/printer_data/config/spd_status.json` khi kết nối/ngắt kết nối socket
2. Moonraker custom component (`spd_status.py`) đọc file này mỗi 5 giây
3. Component gửi lệnh `M117` đến Klipper qua Moonraker internal API
4. Fluidd/Mainsail hiển thị nội dung trên thanh tiêu đề

**Nội dung hiển thị:**

| Trạng thái | Hiển thị |
|------------|----------|
| Đã kết nối | `SPD ID:{machine_id} \| ACTIVE` |
| Mất kết nối | `SPD ID:{machine_id} \| INACTIVE` |
| Chưa có file | `SPD: Disconnected` |

> **Lưu ý**: Tính năng này yêu cầu Moonraker custom component được cài đặt. Script `install.sh` sẽ tự động copy file [`scripts/moonraker_spd_status.py`](scripts/moonraker_spd_status.py) vào thư mục `~/moonraker/moonraker/components/spd_status.py` và thêm `[spd_status]` vào `moonraker.conf`.
...
**1. Xóa Moonraker SPD status component:**

Script uninstall đã xóa file `~/moonraker/moonraker/components/spd_status.py`. Bạn cần xóa thủ công cấu hình `[spd_status]` trong `moonraker.conf`:
...
Sau đó khởi động lại Moonraker:
```bash
sudo systemctl restart moonraker
```

**2. Xóa macro khỏi `printer.cfg`:**
...
**3. Xóa update manager khỏi `moonraker.conf`:**
...
**4. Xóa `spdklipper-plugin` khỏi `moonraker.asvc`:**
...
│   ├── moonraker_spd_status.py # Moonraker component hiển thị trạng thái SPD qua M117
...
| Không thấy hiển thị trạng thái SPD trên Fluidd/Mainsail | Moonraker chưa được cài component | Chạy `ls ~/moonraker/moonraker/components/spd_status.py` để kiểm tra. Nếu thiếu, copy thủ công: `cp ~/spdklipper-plugin/scripts/moonraker_spd_status.py ~/moonraker/moonraker/components/spd_status.py` |
| Không thấy hiển thị trạng thái SPD trên Fluidd/Mainsail | Thiếu `[spd_status]` trong `moonraker.conf` | Thêm `[spd_status]` vào `moonraker.conf` và restart Moonraker |
